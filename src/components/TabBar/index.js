...
function Tab({
  ...
  const content = (
    <div className="relative md:min-w-[90px] w-full cursor-pointer" onClick={onClick}>
      <div
        className={clsx(
          "hover:bg-[#f0f2f5] w-full py-2  rounded-md flex items-center justify-center",
          active && "hover:bg-transparent ",displayTitle && "gap-x-2"
        )}
      >
        <Icon
        ...
        />
        {displayTitle ? <span className={clsx("text-blue-500", !active && "text-gray-500")}>{title}</span> : null}
      </div>
      {active ? (
